        ...
        {selectedTask && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
            >
              {/* Sticky Header */}
              ...
              {selectedTask.status === "Pending Approval" && (
                <div className="p-6 border-t bg-gray-50 flex flex-col sm:flex-row gap-3 flex-shrink-0">
                  <button
                    onClick={() =>
                      verifyTask(selectedTask._id, "approve")
                    }
                    className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-3.5 rounded-2xl font-semibold transition-all active:scale-[0.98]"
                  >
                    ✅ Mark as Resolved
                  </button>
                  ...
                  <button
                    onClick={() =>
                      verifyTask(selectedTask._id, "reject")
                    }
                    className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3.5 rounded-2xl font-semibold transition-all active:scale-[0.98]"
                  >
              ...
              {selectedTask.status === "Unable To Complete" && (
                <div className="p-6 border-t bg-red-50 flex flex-col sm:flex-row gap-3 flex-shrink-0">
                  <button
                    onClick={() =>
                      verifyTask(selectedTask._id, "reassign")
                    }
                    className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-2xl font-semibold transition-all"
                  >
                    🔄 Reassign Again
                  </button>
                  ...
                  <button
                    onClick={() =>
                      verifyTask(selectedTask._id, "move-to-pending")
                    }
                    className="flex-1 bg-amber-600 hover:bg-amber-700 text-white py-3.5 rounded-2xl font-semibold transition-all"
                  >
                    📌 Move To Pending
                  </button>
                </div>
...
const ImageCard = ({ title, image }) => (
  <div>
    <p className="font-semibold mb-2 text-gray-800 text-sm">{title}</p>
    <div
